...
class UserDriftDataSource implements UserLocalDataSource {
  ...
  Future<void> upsertUser(domain.User user) async {
    final existing = await (_db.select(
      _db.users,
    )..where((t) => t.uuid.equals(user.uuid))).getSingleOrNull();
  ...
  Future<void> updateName(String name) async {
    ...
    await (_db.update(_db.users)..where((t) => t.id.equals(existing.id))).write(
      db.UsersCompanion(name: Value(name)),
    );
  }
  ...
  Future<void> updateColor(String color) async {
    ...
    await (_db.update(_db.users)..where((t) => t.id.equals(existing.id))).write(
      db.UsersCompanion(color: Value(color)),
    );
  }
  ...
  Future<void> setLocalNodeId(int nodeId) async {
    ...
    if (existing == null) {
      throw StateError(
        'No hay perfil de usuario creado. '
        'No se puede asociar el nodo local.',
      );
    }

    await (_db.update(_db.users)..where((t) => t.id.equals(existing.id))).write(
      db.UsersCompanion(localNodeId: Value(nodeId)),
    );
  }
  ...
  db.UsersCompanion _toCompanion(domain.User user) {
    return db.UsersCompanion(
      ...
      localNodeId: Value(user.localNodeId),
    );
  }
